/**
 * ARCANO OS - Versión Estable Original (Restaurada)
 * Sincronizada con la Interfaz Actual
 */

document.addEventListener('DOMContentLoaded', () => {
    // --- Referencias a la UI Actual ---
    ...
    const setStatus = (status) => {
        if (status === 'active') {
            ...
            startBtn.innerHTML = `<i class="fas fa-stop"></i> ${lang === 'es' ? 'FINALIZAR SESIÓN' : 'STOP SESSION'}`;
        } else {
            botContainer.classList.remove('status-active');
            startBtn.innerHTML = `<i class="fas fa-terminal"></i> ${lang === 'es' ? 'INICIAR CONSULTORÍA' : 'START CONSULTANCY'}`;
        }
    };

    // ─── AI Voice Visualizer ───
    function animateOrbFromAI() {
        if (!aiAnalyser || !isActive) return;
    ...
    function playPCM(base64) {
        ...
        try {
            ...
            src.start(nextPlayTime || nextAudioTime); // Compatibility fix
    ...
    if (visionBtn) {
        visionBtn.addEventListener('click', () => {
            // Lógica de visión simplificada para mantener estabilidad
            alert("Visión activada en modo estable.");
        });
    }
});
